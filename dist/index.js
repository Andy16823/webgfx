...
// src/core/Camera.ts
var WORLD_FRONT = vec3.fromValues(0, 0, -1);
var WORLD_UP = vec3.fromValues(0, 1, 0);
var WORLD_RIGHT = vec3.fromValues(1, 0, 0);
var PerspectiveCamera = class {
  ...
  getCameraPositionVec4() {
    return vec4.fromValues(this.position[0], this.position[1], this.position[2], 1);
  }
...
import { mat4 as mat42, vec3 as vec32, quat as quat2 } from "gl-matrix";
var Transform = class {
  /**
   ...
   */
  constructor(position = vec32.create(), rotation = quat2.create(), scale = vec32.fromValues(1, 1, 1)) {
    this.position = position;
    this.rotation = rotation;
    this.scale = scale;
  }
  ...
  setRotationEuler(euler) {
    const q = quat2.create();
    quat2.fromEuler(q, euler[0], euler[1], euler[2]);
    this.rotation = q;
  }
  /**
   ...
   */
  getModelMatrix() {
    const modelMatrix = mat42.create();
    mat42.fromRotationTranslationScale(modelMatrix, this.rotation, this.position, this.scale);
    return modelMatrix;
  }
};
...
var GFXMesh = class {
  ...
  constructor(name) {
    ...
    this.indexCount = 0;
    this.materialIndex = -1;
    this.position = vec33.create();
    this.rotation = quat3.create();
...
import { quat as quat4, vec3 as vec34 } from "gl-matrix";
...
import { vec3 as vec35, vec4 as vec42 } from "gl-matrix";
var MeshBuilder = class {
  ...
  addVertex3(position, normal, uv) {
    const tangent = vec42.fromValues(1, 0, 0, 1);
  ...
  addVertex2(position, uv) {
    const normal = vec35.fromValues(0, 0, 1);
...
function Viewport({ scene, invalidateSignal, width = 800, height = 600, mode = 1 /* OnDemand */, onKeyDown, onMouseMove, onMouseDown }) {
  ...
  return /* @__PURE__ */ jsx("canvas", { ref: canvasRef, width, height });
}
...
var GFXFont = class _GFXFont {
   ...
   */
  createTexture(gfx, width, height) {
   ...
   */
  copyDataToTexture(gfx, data, width, height) {
    gfx.device.queue.writeTexture(
      {
   ...
   */
  static async loadFont(gfx, fontUrl, size = 72) {
    var _a;
    ...
    let penY = 0;
    let rowHeight = 0;
    const gfxFont = new _GFXFont(size);
    ...
    for (let i = 0; i < glyphCount; i++) {
      ...
      if (glyph.xMin == null || glyph.xMax == null || glyph.yMin == null || glyph.yMax == null) {
        gfxFont.glyphs.set(char, {
          ...
          bearingX: 0,
          bearingY: 0,
          advance: ((_a = glyph.advanceWidth) != null ? _a : 0) * scale,
          u0: 0,
          v0: 0,
          u1: 0,
          v1: 0
  ...
  createBufferDataForText(text, posX, posY) {
    ...
    for (const char of text) {
      ...
      const y0 = cursorY + glyph.bearingY - glyph.height;
   ...
   */
  createBindGroup(gfx, pipeline, bindingIndex) {
